...
function isValidShip(ship) {
  if (!Array.isArray(ship) || ship.length < 2 || ship.length > 5) return false;

  let isRowAligned = true;
  let isColAligned = true;

  for (let i = 1; i < ship.length; i++) {
    if (
      ship[i].row !== ship[i - 1].row ||
      ship[i].col !== ship[i - 1].col + 1
    ) {
      isRowAligned = false;
    }
    if (
      ship[i].col !== ship[i - 1].col ||
      ship[i].row !== ship[i - 1].row + 1
    ) {
      isColAligned = false;
    }
  }
...
function drawEmptyBoard() {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      emptyMarkers.push({ row, col, mark: "_" });
    }
  }

  displayMarkersOnGrid(emptyMarkers);
  /* TODO */
}
...
function hasLost(player) {
  for (let i = 0; i < player.ships.length; i++) {
    const shipCoord = player.ships[i];
    let isHit = false;

    for (let j = 0; j < player.hits.length; j++) {
      const hitCoord = player.hits[j];
      if (shipCoord.row === hitCoord.row && shipCoord.col === hitCoord.col) {
        isHit = true;
        break;
      }
    }
...
function registerHitOrBom(guess, player) {
  ...
  for (let i = 0; i < player.ships.length; i++) {
    const shipCoord = player.ships[i];
    if (shipCoord.row === guess.row && shipCoord.col === guess.col) {
      isHit = true;
      break;
    }
  }
